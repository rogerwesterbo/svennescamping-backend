# Makefile for svennescamping-backend

# Variables
BINARY_NAME=svennescamping-backend
GO=go
GOFMT=gofmt
DOCKER=docker
KUBECTL=kubectl
DOCKER_IMAGE=ghcr.io/rogerwesterbo/svennescamping-backend
DOCKER_TAG=latest
...
##@ Build
.PHONY: all build clean test lint fmt vet run docker-build docker-push docker-buildx docker-multiarch docker-buildx-push docker-inspect-multiarch help
...
##@ Docker
.PHONY: docker-build docker-push docker-buildx docker-buildx-push docker-multiarch

docker-build: check-docker ## Build Docker image for current platform
	@echo "${CYAN}Building Docker image for current platform...${RESET}"
...
docker-buildx: check-docker ## Setup Docker buildx for multi-architecture builds
	@echo "${CYAN}Setting up Docker buildx for multi-architecture builds...${RESET}"
	@${DOCKER} buildx ls | grep -q svennescamping-builder || \
		${DOCKER} buildx create --name svennescamping-builder --platform linux/amd64,linux/arm64
	@echo "${GREEN}Docker buildx builder 'svennescamping-builder' is ready${RESET}"

docker-multiarch: docker-buildx ## Build multi-architecture Docker images (amd64, arm64)
	@echo "${CYAN}Building multi-architecture Docker images...${RESET}"
	${DOCKER} buildx build \
		--builder svennescamping-builder \
...
docker-buildx-push: docker-buildx ## Build and push multi-architecture Docker images
	@echo "${CYAN}Building and pushing multi-architecture Docker images...${RESET}"
	${DOCKER} buildx build \
		--builder svennescamping-builder \
		--platform linux/amd64,linux/arm64 \
		--tag ${DOCKER_IMAGE}:${DOCKER_TAG} \
...
docker-inspect-multiarch: ## Inspect multi-architecture image manifest
	@echo "${CYAN}Inspecting multi-architecture image manifest...${RESET}"
